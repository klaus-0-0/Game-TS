...
const createGame = async (req, res) => {
    try {
        ...
        // fetch user wallet
        const wallet = await getUserWallet(userId);
        const { mines, stake, gameId } = req.body;
        if (mines <= 0)
            return res.status(404).json({ message: "no mines detected" });
...
const clickTile = async (req, res) => {
    try {
        ...
        // fetch user wallet
        const wallet = await getUserWallet(userId);
        const { clickedTile, gameId } = req.body;
...
//# sourceMappingURL=diamondGameLogic.js.map